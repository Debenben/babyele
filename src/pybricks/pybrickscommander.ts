...
export class PybricksCommander extends BluetoothHciSocket implements CommanderAbstraction {
  ...
  constructor(dog: SensorAbstraction) {
    ...
    this.on('data', function(data) {
      ...
      if(id < 1 || id > 6) return;
      const status = data.readUInt8(19);
      const motorStatus = this.dog.motorStatus;
      if(id == 5) this.dog.notifyDogAcceleration([data.readInt16LE(20), data.readInt16LE(22), data.readInt16LE(24)]);
      const motorAngles = this.dog.motorAngles;
      if(id < 5) {
        /*if(motorStatus[3*id - 2] != (status & 0b00000010 == 0b00000010)) {
          motorStatus[3*id - 2] = (status & 0b00000010 == 0b00000010);
	  this.dog.notifyMotorStatus(motorStatus);
        }
        const accelerometerStatus = this.dog.accelerometerStatus;
        if(accelerometerStatus[2*id - 1] != (status & 0b00000100 == 0b00000100)) {
          accelerometerStatus[2*id - 1] = (status & 0b00000100 == 0b00000100);
	  this.dog.notifyAccelerometerStatus(accelerometerStatus);
        }*/
        const topAcceleration = this.dog.topAcceleration;
	const bottomAcceleration = this.dog.bottomAcceleration;
	topAcceleration[id - 1] = [data.readInt16LE(20), data.readInt16LE(22), data.readInt16LE(24)];
        motorAngles[id - 1][2] = 10*data.readInt16LE(26);
	bottomAcceleration[id - 1] = [data.readInt16LE(28), data.readInt16LE(30), data.readInt16LE(32)];
        this.dog.notifyTopAcceleration(topAcceleration);
        this.dog.notifyBottomAcceleration(bottomAcceleration);
      }
      else  {
        motorAngles[2*(id - 5)][0] = 10*data.readInt16LE(26);
        motorAngles[2*(id - 5)][1] = 10*data.readInt16LE(28);
        motorAngles[2*(id - 5) + 1][0] = 10*data.readInt16LE(30);
        motorAngles[2*(id - 5) + 2][1] = 10*data.readInt16LE(32);
      }
      this.dog.notifyMotorAngles(motorAngles);
      // this.emit('observe', {channel: data.readUInt8(18), data: data.slice(19, data.length -1), rssi: data.readInt8(data.length -1)})
    });
  ...
  async connect() {
    ...
    var filter = Buffer.allocUnsafe(14).fill(0);
  ...
  setAdvertiseParameters() {
    var cmd = Buffer.allocUnsafe(19).fill(0);
    cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  ...
  setAdvertiseData(data) {
    var cmd = Buffer.allocUnsafe(36).fill(0);
    cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  ...
  setAdvertiseEnable(enabled) {
    var cmd = Buffer.allocUnsafe(5).fill(0);
    cmd.writeUInt8(HCI_COMMAND_PKT, 0);
